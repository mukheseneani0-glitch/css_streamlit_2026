...
if len(edited_df) >= 2:
    ...
    corr, _ = pearsonr(edited_df["Practical Hours"], edited_df["Post (%)"])
    ...
    with tab2:
        fig2 = px.bar(edited_df.assign(Improvement=improve), x="Group", y="Improvement",
                      title="Improvement (Post - Pre)", color="Improvement")
        st.plotly_chart(fig2, use_container_width=True)

    with tab3:
        fig3 = px.scatter(edited_df, x="Practical Hours", y="Post (%)", size="Students",
                          hover_name="Group", trendline="ols",
                          title="Practical Hours vs Post-test Score")
